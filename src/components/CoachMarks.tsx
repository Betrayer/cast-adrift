...
import { useViewport } from '@/app/breakpoints';
...
const sameCoachRect = (a: CoachRect, b: CoachRect): boolean =>
  a.x === b.x && a.y === b.y && a.w === b.w && a.h === b.h;

export const nextCoachTarget = (
  prev: CoachTarget | null,
  mark: CoachMarkDef,
  rect: CoachRect,
): CoachTarget =>
  prev !== null && prev.mark.id === mark.id && sameCoachRect(prev.rect, rect)
    ? prev
    : { mark, rect };
...
const CoachMarkHost = ({ screen }: { screen: ScreenId }) => {
  ...
  useEffect(() => {
    const evaluate = (): void => {
      ...
      setActive((prev) => nextCoachTarget(prev, mark, rect));
    };
